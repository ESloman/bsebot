...
class AutoGeneratedBets(BestSummerEverPointsDB):
    ...
    def insert_new_category(self, category: str):
    ...
    def get_all_bets(self) -> list:
        ...
        rets = [r for r in rets if not r.get("archived")]
        return rets

    def get_bet_categories(self) -> list:
        """_summary_

        Returns:
            list: _description_
        """
        categories = self.vault.distinct("type", {"archived": {"$ne": True}})
        return categories
    ...
    def get_bets_for_type(self, scenario_type: str) -> list:
        ...
        :return:
        """
        rets = self.query({"type": scenario_type, "selectable": {"$ne": False}})
        return rets
...
class SpoilerThreads(BestSummerEverPointsDB):
    ...
    def get_all_threads(self, guild_id: int) -> list[Thread]:
        ...
        ret = [Thread(**thread) for thread in ret]
        return ret
